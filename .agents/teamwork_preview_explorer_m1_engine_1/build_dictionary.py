# -*- coding: utf-8 -*-
"""
Dictionary Generator and Validator for Antigravity Chinese Patch
Builds 400+ key-value pairs categorized across all UI modules.
Validates uniqueness, checks for typos, and generates pure ASCII Unicode escapes.
"""
...
# Comprehensive dictionary mapping categorized by domain
ENTRIES = {
    # =========================================================================
...
def validate_and_analyze():
    print(f"Total compiled dictionary keys: {len(ENTRIES)}")
    ...
    for k, v in ENTRIES.items():
        if "修政" in v:
            typos_found.append((k, v))
        if "智能体" not in v and "智能体" in k:
            pass
    if typos_found:
        print(f"ERROR: Typos found: {typos_found}")
    else:
        print("Validation Passed: Zero typos detected ('已修政' completely eliminated).")

    # Generate JSON
    with open("compiled_dictionary.json", "w", encoding="utf-8") as f:
        json.dump(ENTRIES, f, ensure_ascii=False, indent=2)
    print("Exported compiled_dictionary.json (Clean UTF-8)")

    # Generate ASCII Escaped JS object
    js_lines = ["const dictionary = {"]
    for k, v in ENTRIES.items():
        escaped_key = to_unicode_escapes(k)
        escaped_val = to_unicode_escapes(v)
        # Escape quotes in key/val if any
        escaped_key = escaped_key.replace('"', '\\"')
        escaped_val = escaped_val.replace('"', '\\"')
        js_lines.append(f'  "{escaped_key}": "{escaped_val}",')
    ...
    js_output = "\n".join(js_lines)
    with open("compiled_dictionary_escaped.js", "w", encoding="ascii") as f:
        f.write(js_output)
    print("Exported compiled_dictionary_escaped.js (Pure 7-bit ASCII Unicode Escaped)")

    # Verify that escaped file is 100% pure ASCII
    is_pure_ascii = True
    for c in js_output:
        if ord(c) > 127:
            is_pure_ascii = False
            break
    print(f"100% Pure ASCII Verification: {'PASSED [OK]' if is_pure_ascii else 'FAILED [ERROR]'}")
